      ...
      <el-table-column label="操作" width="310">
        <template slot-scope="scope">
          <el-button-group>
            ...
            <el-button size="mini" type="primary" v-if="!((scope.row.preStatus===5 && scope.row.isDill === 2)||scope.row.preStatus===2||scope.row.preStatus===1)" @click="operationOther({ id: scope.row.id, preStatus: 3 }, '确认撤回吗', '/dr/preplan/changePreplanStatus.do')">撤回</el-button>
          </el-button-group>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="planTitle" width="800px" :visible.sync="formShow" :modal-append-to-body="false" @close="closeDialog('formAll')">
      <el-form :model="form" ref="formAll" label-position="right" label-width="110px">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="审核流程：" prop="approveTemplateId" :rules="[
                { required: true, message: '请选择审核流程', trigger: 'change' }
              ]">
              <el-select v-model="form.approveTemplateId" placeholder="请选择审核流程" style="width:100%;">
